from gardenflowerspider.items import GardenLeaf
import datetime
import scrapy
import time
    ...
    def parse(self, response):
        ...
        if ((len(url) == 20) or (current_page == last_page)):
            for leavesset in url:
                ...
                leaf_name = extract_whole_text(leavesset.css('a ::text'))

                yield GardenLeaf(leaf_name=leaf_name, page=current_page, file_urls=[imageURL])

        
        # reload page if there are less then 20 pictures and it is not the last
        # page
        if ((len(url) < 20) and (current_page != last_page)) :
            next_page = 'https://garden.org/apps/plant_photos/view/forever/popular/0/?q_caption=&q_gallery=leaves&offset=' + str(20*(current_page-1))
